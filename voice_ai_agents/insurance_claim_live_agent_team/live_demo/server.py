...
from settings import has_credentials, is_azure, live_model_id, voice_mode  # noqa: E402

GENAI_CLIENT = None
logger = logging.getLogger(__name__)
logging.basicConfig(level=os.getenv("LOG_LEVEL", "INFO"), format="%(asctime)s %(levelname)s %(name)s %(message)s")
FRAME_MAX_AGE_SECONDS = 12.0
AZURE_HISTORY_LIMIT = 40
...
def health() -> dict[str, Any]:
    return {
        ...
        "provider": "azure_openai" if is_azure() else "gemini",
...
async def _draw_incident_sketch(session: IntakeSession, args: dict[str, Any]) -> dict[str, Any]:
    """Generate a pen sketch of the incident scene with the image model, or SVG on Azure."""
    ...
    if is_azure():
        from azure_backend import draw_svg_sketch

        svg = await draw_svg_sketch(brief)
        version = (session.sketch or {}).get("version", 0) + 1
        session.sketch = {
            "data_url": _data_url(svg, "image/svg+xml"),
            "brief": brief,
            "version": version,
            "confirmed": False,
        }
...
def _fresh_frame(session: IntakeSession) -> bytes | None:
    if session.last_frame is None:
        return None
    if time.monotonic() - session.last_frame_at > FRAME_MAX_AGE_SECONDS:
        return None
...
def _azure_user_message(text: str, frame: bytes | None) -> dict[str, Any]:
    if frame is None:
    ...
    return {
        ...
        "content": [
            ...
            {
                "type": "image_url",
                "image_url": {"url": _data_url(frame, "image/jpeg"), "detail": "low"},
            },
        ],
    }


def _trim_azure_messages(session: IntakeSession) -> None:
    """Keep the live prompt short without cutting a tool-call in half."""

    messages = session.azure_messages
    if len(messages) <= AZURE_HISTORY_LIMIT:
        return
    system, rest = messages[0], messages[1:]
    rest = rest[-(AZURE_HISTORY_LIMIT - 1) :]
    while rest and rest[0].get("role") == "tool":
        rest = rest[1:]
    session.azure_messages = [system, *rest]


async def _run_azure_live(websocket: WebSocket, session: IntakeSession) -> None:
    """Turn-based Azure OpenAI stand-in for Gemini Live (text, vision, tools)."""

    import json as json_lib

    from azure_backend import azure_client, openai_tool_declarations
    from settings import azure_deployment

    session.azure_messages = [{"role": "system", "content": SYSTEM_INSTRUCTION}]
    state_lock = asyncio.Lock()
    turn_lock = asyncio.Lock()
    background_tasks: set[asyncio.Task] = set()
    live_model = session.live_model or live_model_id()
    ...
    async def run_named_tool(name: str, args: dict[str, Any], call_id: str) -> dict[str, Any]:
        ...
        if urgent:
            result = {**result, "interrupt": True, "instruction": "Stop and escalate this to a human now."}
        return result

    async def run_model_turn(user_text: str, *, include_frame: bool, record_claimant: bool) -> None:
        async with turn_lock:
            if record_claimant:
                async with state_lock:
                    session.transcript.append({"speaker": "Claimant", "text": user_text})
            frame = _fresh_frame(session) if include_frame else None
            session.azure_messages.append(_azure_user_message(user_text, frame))
            pending_output = ""
            client = azure_client()
            ...
            for _round in range(8):
                ...
                if tool_acc:
                    ...
                    session.azure_messages.append(assistant_msg)
                    for slot in assistant_tool_calls:
                        try:
                            args = json_lib.loads(slot["function"]["arguments"] or "{}")
                            if not isinstance(args, dict):
                                args = {}
                        except json_lib.JSONDecodeError:
                            args = {}
                        result = await run_named_tool(slot["function"]["name"], args, slot["id"])
                        session.azure_messages.append(
                            {
                                "role": "tool",
                                "tool_call_id": slot["id"],
                                "content": json_lib.dumps(result, ensure_ascii=False, default=str),
                            }
                        )
                    continue

                if content.strip():
                    session.azure_messages.append({"role": "assistant", "content": content})
                    session.transcript.append({"speaker": "Agent", "text": content.strip()})
                    await websocket.send_json(
                        {
                            "type": "transcript",
                            "speaker": "Agent",
            ...
            _trim_azure_messages(session)
            if record_claimant:
                track(asyncio.create_task(update_claim_state(user_text)))
    ...
    try:
        while True:
            ...
            if msg_type == "audio":
                continue
            if msg_type == "video":
                session.last_frame = base64.b64decode(message["data"])
                session.last_frame_at = time.monotonic()
            elif msg_type == "look":
                await run_model_turn(
                    "I'm showing you the live camera now. Describe only what you can actually see in this frame. "
                    "If it is relevant to the claim, pin it.",
                    include_frame=True,
                    record_claimant=False,
                )
            elif msg_type == "text":
                text = str(message.get("text", "")).strip()
                if text:
                    await websocket.send_json(
                        {"type": "transcript", "speaker": "Claimant", "text": text, "final": True}
                    )
                    await run_model_turn(text, include_frame=True, record_claimant=True)
            elif msg_type == "close":
        ...
        logger.exception("Azure live session failed")
        try:
            await websocket.send_json({"type": "error", "message": f"Azure OpenAI session failed: {exc}"})
        except Exception:
            pass
    finally:
...
async def live_voice(websocket: WebSocket) -> None:
    ...
    if not _has_api_key():
        missing = (
            ...
            if is_azure()
            else "Missing GOOGLE_API_KEY."
        )
        await websocket.send_json(
            {
                "type": "error",
                "message": f"{missing} Add it to {APP_DIR / '.env'} and restart the backend.",
            }
        )
        await websocket.close()
        return
    ...
    await websocket.send_json(
        {
            ...
            "tools": TOOL_NAMES,
            "message": (
                "Azure OpenAI claim session connected. Voice uses the browser; the model sees camera stills."
                if is_azure()
                else "Gemini 3.8 Live voice session connected."
            ),
        }
    )

    if is_azure():
        await _run_azure_live(websocket, session)
        return
