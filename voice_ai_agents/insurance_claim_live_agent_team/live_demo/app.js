...
async function connectLive() {
  ...
  liveSocket.onmessage = (event) => {
    ...
    if (message.type === "session") {
      ...
      if (message.voice_mode) voiceMode = message.voice_mode;
      ...
      if (message.speaker === "Agent" && message.final) speakAgent(message.text);
  ...
  try {
    ...
    lookTimer = window.setTimeout(() => {
      if (liveSocket && liveSocket.readyState === WebSocket.OPEN) {
        liveSocket.send(JSON.stringify({ type: "look" }));
      }
    }, 1200);
...
function speakAgent(text) {
  if (voiceMode !== "browser" || !window.speechSynthesis || !String(text || "").trim()) return;
  window.speechSynthesis.cancel();
  const utterance = new SpeechSynthesisUtterance(text);
  utterance.lang = /[\u0900-\u097F]/.test(text) ? "hi-IN" : navigator.language || "en-US";
...
async function startBrowserVoice() {
  ...
  if (!SpeechRecognition) {
    appendSystem("This browser has no speech recognition. Use Chrome or Edge, or type instead.");
    return;
  }
  try {
    await connectLive();
    ...
    recognition.onresult = (event) => {
      ...
      if (interim) upsertStreamingTurn("Claimant", interim, false);
      if (finalText.trim()) {
        window.speechSynthesis?.cancel();
        liveSocket.send(JSON.stringify({ type: "text", text: finalText.trim() }));
        upsertStreamingTurn("Claimant", finalText.trim(), true);
        setWriting(true);
      }
    };
    recognition.onerror = (event) => {
      if (event.error === "not-allowed") {
        appendSystem("Microphone access was denied. Allow it for this site or type instead.");
        stopLiveVoice(false);
      }
    };
    recognition.onend = () => {
    ...
    appendSystem(`Voice failed: ${error.message}`);
    stopLiveVoice(false);
  }
}
