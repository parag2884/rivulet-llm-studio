...
try:
    from .settings import chat_model_id, is_azure
except ImportError:
    from settings import chat_model_id, is_azure
...
async def _run_azure_claim_workflow(transcript: str) -> dict[str, Any]:
    """Same claim graph as ADK, with Azure OpenAI for the two LLM steps."""

    try:
        from .azure_backend import classify_claim, extract_claim
    except ImportError:
        from azure_backend import classify_claim, extract_claim

    claim = await extract_claim(transcript)
    validation = validate_required_claim_fields(claim)
    classification = await classify_claim(claim.model_dump(exclude_none=True), validation)
    coverage = apply_coverage_and_evidence_rules(claim, validation, classification)
    checklist = generate_document_checklist(claim, classification, coverage)
    fraud_gate = fraud_signal_and_safety_gate(claim, validation, classification, coverage)
    packet = build_claim_intake_packet(
    ...
    return _workflow_payload(claim, validation, classification, coverage, checklist, fraud_gate, packet)
...
async def run_claim_workflow(
    ...
    *,
    ...
    """Run the claim graph for the current claimant transcript snapshot."""

    transcript = str(claimant_transcript or "").strip()
    if not transcript:
        return build_initial_workflow_state()
    if is_azure():
        return await _run_azure_claim_workflow(transcript)
    ...
    return _workflow_payload(
        claim,
        validation.model_dump(exclude_none=True),
        classification,
        coverage.model_dump(exclude_none=True),
        checklist.model_dump(exclude_none=True),
        fraud_gate.model_dump(exclude_none=True),
